...
export default function OnboardingForm({ onCreated }: OnboardingFormProps) {
  ...
  async function handleSubmit() {
    ...
    const { data, error: insertError } = await supabase
      .from('contractors')
      .insert({ company_name: companyName.trim(), trade })
      .select()
      .maybeSingle();
    ...
    if (insertError || !data) {
      setError(insertError?.message ?? 'Something went wrong creating your profile. Please try again.');
      return;
    }
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-6 py-12">
      <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl border border-slate-200 p-8">
        ...
        <p className="text-sm text-slate-500 mb-6">
          Tell us a bit about your company so we can configure your quote calculator.
        </p>
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {(Object.entries(TRADES) as [Trade, (typeof TRADES)[Trade]][]).map(
                ([key, config]) => (
                  <button
                    key={key}
                    type="button"
                  ...
                  >
                    <config.icon
                      className={`w-5 h-5 mb-2 ${trade === key ? 'text-blue-600' : 'text-slate-500'}`}
                    />
          ...
          <button
            type="button"
            disabled={loading || !companyName.trim() || !trade}
            onClick={handleSubmit}
            className="w-full py-2.5 rounded-lg bg-blue-600 text-white font-semibold hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
          >
